constitution 原則・ルール・前提

思考は英語で行い、最終的な出力は必ず日本語で提供してください。

## 開発の基本理念
- 運用フローから、データの設定・参照タイミングを確認し、データモデル中心のアプローチを行う。
- 不明瞭、曖昧、要件間の矛盾は、必ず解消する。
- 実装前に画面モックを作成し、レビューを実施する。。
...
- 画面イベントに対し、機能が正しく設定されていることをチェックし、実装する。
- 画面入力項目は、事前に入力した情報が、適切に参照され、容易に入力できるようにする。
- 外部利用者間で、個別の情報が互いに参照できないよう、考慮する。
- テスト完了した時点で、利用者マニュアル、運用手順書を作成する。
